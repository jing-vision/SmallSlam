...
void CoreSLAM::setup()
{
    robotParams.r = 0.077;
    robotParams.R = 0.165;
    robotParams.inc = 2000;
    robotParams.ratio = 1.0;
    
    laserParams.offset = 145;
    laserParams.scan_size = TEST_SCAN_SIZE;
    laserParams.angle_min = -120;
    laserParams.angle_max = +120;
    laserParams.detection_margin = 70;
    laserParams.distance_no_detection = 4000;
...
void CoreSLAM::update()
{
    ...
    for (loop = 0; loop != nb_loops; loop++) {
        ...
        ts_state_init(&state, &map, &robotParams, &laserParams, &loop_startpos, 100, 20, 600, TS_DIRECTION_FORWARD);
...
#endif
        ts_state_init(&state, &map, &robotParams, &laserParams, &loop_endpos, 100, 20, 600, TS_DIRECTION_BACKWARD);
